...
| `BE-DDMRP-003` | 计算 Net Flow Position | `[VERIFIED]` | `D` `docs/superpowers/specs/2026-07-14-ddmrp-sampleco-runtime-alignment-design.md`; `C` `evaluate_ddmrp_net_flow`; `A` `/planner/workbench/ddmrp/net-flow/evaluate`; `T` `tests/test_ddmrp.py`, `tests/test_ddsop_runtime_planning_input.py`, `tests/test_api.py`；2026-07-14 定向 135 passed、全量 1234 passed | 公式为合格在手 + 有效开放供应 - 合格需求；DDS&OP 已确认尖峰不受普通 DLT 窗口二次截断，冻结快照内有效开放供应不复用 DLT 作为截止窗口，已收货供应不重复计入；SDBR 侧尖峰计算在缺少已验收阈值权威时拒绝 |
| `BE-DDMRP-004` | 区分计划缓冲状态与在手执行状态 | `[VERIFIED]` | `D` `docs/superpowers/specs/2026-07-14-ddmrp-sampleco-runtime-alignment-design.md`; `C` `PlanningStatus`、`ExecutionStatus`、`PlanningPriorityPercent`、`ExecutionPriorityPercent`; `T` `tests/test_ddmrp.py`, `tests/test_ddmrp_replenishment.py`, `tests/test_ddmrp_replenishment_view.py`；2026-07-14 定向 135 passed、全量 1234 passed | 计划状态基于净流位置，执行状态基于合格在手库存，二者不得混用；计划优先级为净流位置/绿区顶部，执行优先级为合格在手/红区顶部；零分母返回空值；2.86 前不可变评估行可在通过原始指纹校验后由只读层派生这两个指标，不改写历史记录 |
...
| 版本 | 日期 | 变更 |
| --- | --- | --- |
| 2.87 | 2026-07-14 | 完成 `BE-DDMRP-004/007` 历史评估兼容修复：2.86 新增计划/执行优先级后，旧不可变评估行若仅缺少这两个派生字段，先通过旧行原始指纹校验，再由只读工作台派生显示；未清库、未改写历史记录，也未放宽其他字段校验。验证：兼容回归 2 passed、DDMRP 定向 137 passed、全量 1236 passed，1 个既有 Starlette 弃用警告。 |
| 2.86 | 2026-07-14 | 完成 `SampleCowithDemandDrivenMRP.pdf` 运行算法对齐：`BE-DDMRP-003/004/006` 恢复 `[VERIFIED]`；DDS&OP 已确认尖峰不受普通 DLT 二次截断，有效开放供应不复用 DLT 截止且已收货供应不重复计入，计划优先级按净流位置/绿区顶部、执行优先级按合格在手/红区顶部计算，并在契约适配器和独立评估中校验 UOM 一致性。ADU/DLT/Buffer Profile 参数治理仍归 DDAE。验证：DDMRP 定向 135 passed，全量 1234 passed，1 个既有 Starlette 弃用警告。 |
| 2.85 | 2026-07-13 | 调整 `BE-SDBR-010` 接单确认边界：计划员接受 MTO 建议后，系统创建 Phase 0 预留并排入一个冻结该预留批次的内部 OR-Tools Planning Run；仍不确认外部订单、不修改 DDAE 或 ERP/MES/生产权威。扩充 `BE-DDMRP-007` 受控测试数据以覆盖红、黄、绿和高于绿区各 3 条，状态与生产权威边界不变。 |
