...
| `BE-SDBR-006` | Shared demand commitment identity | `[PARTIAL]` | `C` `sdbr/planning_commitments.py`; `T` `tests/test_planning_commitments.py` | MTO、MTA、DependentDemand 和外部正式订单使用统一需求身份、版本、幂等键和来源分类；业务指纹按 UTC 规范化且不包含每次尝试的 TraceID；注册重放必须分别重新计算并验证候选与持久化指纹，任一记录无法规范化时返回结构化迁移/冲突，不得把两个规范化失败视为相等；相同业务键不得产生两个活动承诺 |
...
| `BE-RUN-011` | Planning Run freezes and converts planning reservations | `[PARTIAL]` | `C` `sdbr/planning_run_reservation_bridge.py`, `sdbr/api.py`, `sdbr/state_store.py`; `T` `tests/test_planning_run_reservation_bridge.py`, `tests/test_api.py`, `tests/test_state_store.py` | Planning Run 以当前格式标记显式冻结 demand/batch/capacity/material identity、record version、content fingerprint 和 lifecycle，并在转态时 compare-and-set；只允许同时缺失 top-level selection 与 frozen graph 的真实历史 run 绕过，当前空图仍必须验证全部 metadata/orphan/selected-ID equality；direct/worker 必须在求解前用短事务持久化唯一 per-run execution version/token，求解期间不得持有 store write lock，heartbeat 必须可提交，lease recovery 必须使旧 claim 失效；finalization 在短事务内重新加载并合并同 ownership heartbeat，以 execution/lease CAS 原子发布 graph、run schedule/publication state 与 audit；所有持久化字段及 metadata 使用完整 store rollback/state root，相关 readers 只看到完整 pre/post state；SQLite commit 后 backup 失败不得恢复旧内存或报告事务失败；能力状态保持 `[PARTIAL]` |
