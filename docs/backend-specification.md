...
| 版本 | 日期 | 变更 |
| --- | --- | --- |
| 2.81 | 2026-07-11 | `BE-DDMRP-007` 完成不可变运行评估与版本化只读补货建议的可重复验证：本地 Node 语法检查、398 项定向测试、848 项全量测试，以及 seeded/empty/error/403/409 浏览器矩阵均通过；证据见 `.tmp/ddmrp-ui-acceptance/browser-report.md` 及七张命名截图。`BE-DDMRP-008`、`BE-DDMRP-009` 保持 `[NOT-STARTED]`，`CONTRACT-GATE-DDMRP-ACTIVATION-001` 仍关闭。 |
