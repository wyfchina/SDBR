...
第一版必须保留 OR-Tools 与 Simio 的正确产品路径和能力状态。未接通时应明确显示 `未配置` 或 `暂不可用`；已接通的能力必须标明作用边界，不能把 Simio 可选验证误导为排程求解器。
...
**2026-06-24 补充验证：**MES 派工队列补充“生成 MES 派工建议包”动作，页面显示包编号和 Mock 投递状态；设备队列显示现场到达/加工状态、释放状态、建议动作和业务原因。该输出仍为内部 Mock 派工建议，不真实控制 MES，未来 Direct Adapter 或 UNS 适配器必须消费同一派工建议语义。
...
- 体验收口：2026-06-24 MES 派工队列改为按设备/工作中心折叠展示，默认只显示资源名称、可派工数量和预警数量，点击“展开/收起”查看具体工序队列，避免缓冲执行页默认铺开大量设备明细。
