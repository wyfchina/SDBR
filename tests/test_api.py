...
def _seed_active_planning_reservation(
    ...
    *,
    ...
    demand = create_demand_commitment(
        ...
        source_object_id="TST-SO-PHASE0",
        source_object_version="1",
        demand_line_id="1",
        item_or_product_id="TST-FG-A",
        location_id="TST-MAIN",
        quantity=1,
        uom="EA",
        required_at=datetime(2026, 6, 27, 1, tzinfo=timezone.utc),
        demand_class="MTO",
    ...
    demand_id = str(demand["DemandCommitmentID"])
    store.planning_demand_commitments[demand_id] = demand
...
def _completed_execution(**_kwargs) -> dict[str, object]:
    return {
        "Status": "Completed",
...
def test_planning_reservation_run_rejects_unknown_batch():
    store = WorkbenchStateStore()
    ...
    assert "RUN-UNKNOWN-RES" not in store.planning_runs


def test_planning_reservation_run_rejects_ineligible_batch():
    store = WorkbenchStateStore()
    seed_baseline_test_data(store)
    _seed_active_planning_reservation(store, batch_id="PRB-INELIGIBLE")
    store.planning_reservation_batches["PRB-INELIGIBLE"]["Status"] = (
    ...
    response = client.post(
        "/planner/workbench/planning-runs",
        json={
            **_phase0_planning_run_payload("RUN-INELIGIBLE-RES"),
            "PlanningReservationBatchIDs": ["PRB-INELIGIBLE"],
        },
    )
...
@pytest.mark.parametrize(
    ...
    [
        ("ccr_capacity_reservations", "CCR-RES-1", []),
        ("ccr_capacity_reservations", "CCR-RES-1", {"bad": "status"}),
        ("material_planning_allocations", "MPA-1", []),
        ("material_planning_allocations", "MPA-1", {"bad": "status"}),
    ],
)
def test_planning_reservation_run_maps_malformed_child_status_to_invalid_graph(
    collection_name: str,
    record_id: str,
    status: object,
):
    store = WorkbenchStateStore()
    seed_baseline_test_data(store)
    _seed_active_planning_reservation(store, batch_id="PRB-MALFORMED-CHILD-STATUS")
    getattr(store, collection_name)[record_id]["Status"] = status
    client = TestClient(create_app(state_store=store), raise_server_exceptions=False)

    response = client.post(
        "/planner/workbench/planning-runs",
        json={
            **_phase0_planning_run_payload(f"RUN-MALFORMED-STATUS-{record_id}"),
            "PlanningReservationBatchIDs": ["PRB-MALFORMED-CHILD-STATUS"],
        },
    )
...
def test_planning_reservation_run_rejects_invalid_batch_graph_without_mutation():
    store = WorkbenchStateStore()
    seed_baseline_test_data(store)
    _seed_active_planning_reservation(store, batch_id="PRB-INVALID")
    del store.ccr_capacity_reservations["CCR-RES-1"]
    original_batch = dict(store.planning_reservation_batches["PRB-INVALID"])
    client = TestClient(create_app(state_store=store))

    response = client.post(
        "/planner/workbench/planning-runs",
        json={
            **_phase0_planning_run_payload("RUN-INVALID-RES"),
            "PlanningReservationBatchIDs": ["PRB-INVALID"],
        },
    )
    ...
    assert store.planning_reservation_batches["PRB-INVALID"] == original_batch
    assert "RUN-INVALID-RES" not in store.planning_runs


def test_planning_reservation_run_freezes_explicit_deep_snapshot():
    store = WorkbenchStateStore()
    seed_baseline_test_data(store)
    _seed_active_planning_reservation(store, batch_id="PRB-FROZEN")
    client = TestClient(create_app(state_store=store))

    created = client.post(
        "/planner/workbench/planning-runs",
        json={
            **_phase0_planning_run_payload("RUN-FROZEN-RES"),
            "PlanningReservationBatchIDs": ["PRB-FROZEN"],
        },
    )
    store.planning_reservation_batches["PRB-FROZEN"]["CapacityReservationIDs"].append(
        "CCR-LATE"
    )
    store.ccr_capacity_reservations["CCR-RES-1"]["ReservedMinutes"] = 999

    assert created.status_code == 200
    run = created.json()["Data"]["PlanningRun"]
    assert run["PlanningReservationBatchIDs"] == ["PRB-FROZEN"]
    assert run["FrozenPlanningReservations"]["ReservationBatchIDs"] == ["PRB-FROZEN"]
    assert run["FrozenPlanningReservations"]["Batches"][0][
        "CapacityReservationIDs"
    ] == ["CCR-RES-1"]
    assert run["FrozenPlanningReservations"]["CapacityReservations"][0][
        "ReservedMinutes"
    ] == 95


def test_planning_reservation_run_completed_converts_capacity_and_traces_active_material(
    monkeypatch,
):
    store = WorkbenchStateStore()
    seed_baseline_test_data(store)
    _seed_active_planning_reservation(store, batch_id="PRB-COMPLETE")
    captured_payloads = []

    def complete_and_capture(*, payload, **_kwargs):
        captured_payloads.append(payload)
        return _completed_execution()

    monkeypatch.setattr("sdbr.api._execute_planning_run", complete_and_capture)
    client = TestClient(create_app(state_store=store))
    assert client.post(
        "/planner/workbench/planning-runs",
        json={
            **_phase0_planning_run_payload("RUN-WITH-RES"),
            "PlanningReservationBatchIDs": ["PRB-COMPLETE"],
        },
    ).status_code == 200
    ...
    assert captured_payloads[0].PlanningReservationBatchIDs == ["PRB-COMPLETE"]
    assert store.planning_reservation_batches["PRB-COMPLETE"]["Status"] == (
        "ConvertedToScheduledOccupancy"
    )
    ...
    assert material["PlanningRunID"] == "RUN-WITH-RES"
    assert material["LastTransitionAt"] == "2026-06-22T07:56:00+00:00"
    assert material["EventType"] == "PlanningRunCompleted"
    audit = store.audit_events[-1]
    assert audit["Details"]["PlanningReservationsConverted"] == ["PRB-COMPLETE"]


def test_planning_reservation_failed_run_holds_for_error(monkeypatch):
    store = WorkbenchStateStore()
    seed_baseline_test_data(store)
    ...
    assert client.post(
        "/planner/workbench/planning-runs",
        json={
            **_phase0_planning_run_payload("RUN-FAILED-RES"),
            "PlanningReservationBatchIDs": ["PRB-FAILED"],
        },
    ).status_code == 200

    executed = client.post(
        "/planner/workbench/planning-runs/RUN-FAILED-RES/execute",
        json={
            "ExecutedBy": "planner-phase0",
            "StartedAt": "2026-06-22T07:55:00+00:00",
            "CompletedAt": "2026-06-22T07:56:00+00:00",
    ...
    assert executed.json()["Data"]["PlanningRun"]["Status"] == "Failed"
    assert store.planning_reservation_batches["PRB-FAILED"]["Status"] == (
...
@pytest.mark.parametrize(
    ("case_id", "schedule"),
    [
        ("NULL-GANTT", {"GanttRows": None}),
        (
            "NULL-BARS",
            {"GanttRows": [{"ResourceID": "TST_WC_DRUM", "Bars": None}]},
        ),
        (
...
def test_planning_reservation_malformed_schedule_returns_structured_409(
    monkeypatch,
    case_id: str,
    ...
    store = WorkbenchStateStore()
    seed_baseline_test_data(store)
    _seed_active_planning_reservation(store, batch_id=f"PRB-{case_id}")
    monkeypatch.setattr(
        "sdbr.api._execute_planning_run",
        lambda **_kwargs: {
            "Status": "Completed",
            "SolverStatus": "Optimal",
            "SolverMessage": "Malformed evidence fixture.",
    ...
    assert client.post(
        "/planner/workbench/planning-runs",
        json={
            **_phase0_planning_run_payload(run_id),
            "PlanningReservationBatchIDs": [f"PRB-{case_id}"],
        },
    ).status_code == 200
    ...
    assert response.json()["Data"]["PlanningRun"]["Status"] == "Failed"
    assert response.json()["Data"]["Reasons"]["CCR-RES-1"]
    assert store.ccr_capacity_reservations["CCR-RES-1"]["Status"] == (
        "HeldForPlanningError"
    )


@pytest.mark.parametrize("drift", ["status", "content", "version", "fingerprint"])
def test_planning_reservation_demand_drift_returns_structured_conflict(
    monkeypatch,
    drift: str,
):
    store = WorkbenchStateStore()
    seed_baseline_test_data(store)
    demand_id = _seed_active_planning_reservation(
        store,
        batch_id=f"PRB-DEMAND-{drift}",
    )
    client = TestClient(create_app(state_store=store))
    run_id = f"RUN-DEMAND-{drift}"
    assert client.post(
        "/planner/workbench/planning-runs",
        json={
            **_phase0_planning_run_payload(run_id),
            "PlanningReservationBatchIDs": [f"PRB-DEMAND-{drift}"],
        },
    ).status_code == 200
    demand = store.planning_demand_commitments[demand_id]
    if drift == "status":
        demand["Status"] = "Cancelled"
    elif drift == "content":
        demand["Quantity"] = 2.0
    elif drift == "version":
        demand["RecordVersion"] = 2
    else:
        demand["ContentFingerprint"] = "sha256:changed"
    monkeypatch.setattr("sdbr.api._execute_planning_run", _completed_execution)
...
def test_planning_reservation_current_graph_missing_metadata_returns_structured_conflict(
    monkeypatch,
):
    store = WorkbenchStateStore()
    seed_baseline_test_data(store)
    _seed_active_planning_reservation(store, batch_id="PRB-MISSING-METADATA")
    client = TestClient(create_app(state_store=store))
    assert client.post(
        "/planner/workbench/planning-runs",
        json={
            **_phase0_planning_run_payload("RUN-MISSING-METADATA"),
            "PlanningReservationBatchIDs": ["PRB-MISSING-METADATA"],
        },
    ).status_code == 200
    store.planning_runs["RUN-MISSING-METADATA"]["FrozenPlanningReservations"].pop(
        "GraphFingerprint"
    )
    monkeypatch.setattr("sdbr.api._execute_planning_run", _completed_execution)

    response = client.post(
        "/planner/workbench/planning-runs/RUN-MISSING-METADATA/execute",
        json={
            "ExecutedBy": "planner-phase0",
            "StartedAt": "2026-06-22T07:55:00+00:00",
            "CompletedAt": "2026-06-22T07:56:00+00:00",
...
def test_planning_reservation_selected_graph_with_null_batch_ids_fails_closed(
    monkeypatch,
):
    store = WorkbenchStateStore()
    seed_baseline_test_data(store)
    _seed_active_planning_reservation(store, batch_id="PRB-NULL-FROZEN-IDS")
    client = TestClient(create_app(state_store=store))
    assert client.post(
        "/planner/workbench/planning-runs",
        json={
            **_phase0_planning_run_payload("RUN-NULL-FROZEN-IDS"),
            "PlanningReservationBatchIDs": ["PRB-NULL-FROZEN-IDS"],
        },
    ).status_code == 200
    store.planning_runs["RUN-NULL-FROZEN-IDS"]["FrozenPlanningReservations"][
        "ReservationBatchIDs"
    ] = None
    monkeypatch.setattr("sdbr.api._execute_planning_run", _completed_execution)

    response = client.post(
        "/planner/workbench/planning-runs/RUN-NULL-FROZEN-IDS/execute",
        json={
            "ExecutedBy": "planner-phase0",
            "StartedAt": "2026-06-22T07:55:00+00:00",
            "CompletedAt": "2026-06-22T07:56:00+00:00",
...
def test_planning_reservation_explicit_legacy_graph_requires_migration(
    monkeypatch,
):
    store = WorkbenchStateStore()
    seed_baseline_test_data(store)
    _seed_active_planning_reservation(store, batch_id="PRB-LEGACY-GRAPH")
    client = TestClient(create_app(state_store=store))
    assert client.post(
        "/planner/workbench/planning-runs",
        json={
            **_phase0_planning_run_payload("RUN-LEGACY-GRAPH"),
            "PlanningReservationBatchIDs": ["PRB-LEGACY-GRAPH"],
        },
    ).status_code == 200
    frozen = store.planning_runs["RUN-LEGACY-GRAPH"]["FrozenPlanningReservations"]
    frozen["GraphFormat"] = "LegacyUnversionedPlanningReservationGraph"
    monkeypatch.setattr("sdbr.api._execute_planning_run", _completed_execution)

    response = client.post(
        "/planner/workbench/planning-runs/RUN-LEGACY-GRAPH/execute",
        json={
            "ExecutedBy": "planner-phase0",
            "StartedAt": "2026-06-22T07:55:00+00:00",
            "CompletedAt": "2026-06-22T07:56:00+00:00",
    ...
    assert response.status_code == 409
    assert response.json()["Data"]["Status"] == (
        "PlanningReservationGraphMigrationRequired"
    )
    assert response.json()["Data"]["PlanningRun"]["Status"] == "Failed"
    assert store.planning_runs["RUN-LEGACY-GRAPH"]["FrozenPlanningReservations"][
        "GraphFormat"
    ] == "LegacyUnversionedPlanningReservationGraph"
    assert store.ccr_capacity_reservations["CCR-RES-1"]["Status"] == (
        "ActivePlanReservation"
    )


def test_planning_reservation_api_preserves_authority_handoff_after_freeze(
    monkeypatch,
):
    store = WorkbenchStateStore()
    seed_baseline_test_data(store)
    _seed_active_planning_reservation(store, batch_id="PRB-AUTHORITY")
    client = TestClient(create_app(state_store=store))
    assert client.post(
        "/planner/workbench/planning-runs",
        json={
            **_phase0_planning_run_payload("RUN-AUTHORITY"),
            "PlanningReservationBatchIDs": ["PRB-AUTHORITY"],
        },
    ).status_code == 200
    material = store.material_planning_allocations["MPA-1"]
    material.update(
        {
            "Status": "Externalized",
            "RecordVersion": 2,
            "ExternalAllocationRef": "ERP-ALLOC-PHASE0",
            "MaterialSnapshotID": "OPS-AUTHORITY-PHASE0",
            "EventType": "AuthorityAllocationExternalized",
        }
    )
    authority_record = deepcopy(material)
    monkeypatch.setattr("sdbr.api._execute_planning_run", _completed_execution)

    response = client.post(
        "/planner/workbench/planning-runs/RUN-AUTHORITY/execute",
        json={
            "ExecutedBy": "planner-phase0",
            "StartedAt": "2026-06-22T07:55:00+00:00",
            "CompletedAt": "2026-06-22T07:56:00+00:00",
    ...
    assert response.status_code == 200
    assert response.json()["Data"]["PlanningRun"]["Status"] == "Completed"
    assert store.material_planning_allocations["MPA-1"] == authority_record
...
def test_planning_reservation_dead_letter_run_holds_for_error(monkeypatch):
    ...
    assert client.post(
        "/planner/workbench/planning-runs",
        json={
            **_phase0_planning_run_payload("RUN-DEAD-RES"),
            "PlanningReservationBatchIDs": ["PRB-DEAD"],
            "MaxAttempts": 1,
        },
    ).status_code == 200
    assert client.post(
        "/planner/workbench/planning-runs/RUN-DEAD-RES/enqueue",
        json={
    ...
    executed = client.post(
        "/planner/workbench/planning-runs/RUN-DEAD-RES/execute",
        json={
            "ExecutedBy": "worker-phase0",
    ...
    assert executed.json()["Data"]["PlanningRun"]["Status"] == "DeadLetter"
    assert store.planning_reservation_batches["PRB-DEAD"]["Status"] == (
        "HeldForPlanningError"
    )
    assert store.audit_events[-1]["Details"]["PlanningReservationsHeldForError"] == [
        "PRB-DEAD"
...
def test_planning_reservation_queued_retry_does_not_transition_or_claim_audit(
    ...
    assert client.post(
        "/planner/workbench/planning-runs",
        json={
            **_phase0_planning_run_payload("RUN-RETRY-RES"),
            "PlanningReservationBatchIDs": ["PRB-RETRY"],
            "MaxAttempts": 2,
        },
    ).status_code == 200
    assert client.post(
        "/planner/workbench/planning-runs/RUN-RETRY-RES/enqueue",
        json={
            "EnqueuedBy": "planner-phase0",
            "EnqueuedAt": "2026-06-22T07:51:00+00:00",
            "MaxAttempts": 2,
    ...
    executed = client.post(
        "/planner/workbench/planning-runs/RUN-RETRY-RES/execute",
        json={
            "ExecutedBy": "worker-phase0",
            "StartedAt": "2026-06-22T07:52:01+00:00",
            "CompletedAt": "2026-06-22T07:52:10+00:00",
    ...
    assert executed.json()["Data"]["PlanningRun"]["Status"] == "Queued"
    assert store.planning_reservation_batches["PRB-RETRY"]["Status"] == (
        "ActivePlanReservation"
    )
    details = store.audit_events[-1]["Details"]
    assert "PlanningReservationsConverted" not in details
...
def test_planning_reservation_empty_current_graph_is_still_validated(
    ...
    created = client.post(
        "/planner/workbench/planning-runs",
        json=_phase0_planning_run_payload("RUN-NO-RES"),
    )
    executed = client.post(
        "/planner/workbench/planning-runs/RUN-NO-RES/execute",
        json={
            "ExecutedBy": "planner-phase0",
            "StartedAt": "2026-06-22T07:55:00+00:00",
            "CompletedAt": "2026-06-22T07:56:00+00:00",
...
def test_planning_reservation_transition_is_atomic_when_bridge_fails(monkeypatch):
    store = WorkbenchStateStore()
    seed_baseline_test_data(store)
    _seed_active_planning_reservation(store, batch_id="PRB-ATOMIC")
    monkeypatch.setattr("sdbr.api._execute_planning_run", _completed_execution)
    client = TestClient(
        create_app(state_store=store),
        raise_server_exceptions=False,
    ...
    assert client.post(
        "/planner/workbench/planning-runs",
        json={
            **_phase0_planning_run_payload("RUN-ATOMIC-RES"),
            "PlanningReservationBatchIDs": ["PRB-ATOMIC"],
        },
    ).status_code == 200
    original_batch = dict(store.planning_reservation_batches["PRB-ATOMIC"])
    original_capacity = dict(store.ccr_capacity_reservations["CCR-RES-1"])
    original_material = dict(store.material_planning_allocations["MPA-1"])

    def bridge_failure(**_kwargs):
    ...
    response = client.post(
        "/planner/workbench/planning-runs/RUN-ATOMIC-RES/execute",
        json={
            "ExecutedBy": "planner-phase0",
            "StartedAt": "2026-06-22T07:55:00+00:00",
            "CompletedAt": "2026-06-22T07:56:00+00:00",
    ...
    failed_run = store.planning_runs["RUN-ATOMIC-RES"]
    assert failed_run["Status"] == "Failed"
    assert failed_run["ExecutionVersion"] == 1
    assert failed_run["ExecutionClaimedBy"] == "planner-phase0"
    assert failed_run["ExecutionClaimInvalidationReason"] == "FinalizationError"
    assert "ExecutionTokenHash" not in failed_run
    assert "ExecutionClaimExpiresAt" not in failed_run

    recovered = client.post(
        "/planner/workbench/planning-runs/RUN-ATOMIC-RES/recover",
        json={
            "RecoveredBy": "planner-phase0",
            "RecoveredAt": "2026-06-22T07:57:00+00:00",
            "Reason": "Retry after finalization persistence was repaired.",
        },
    )
...
def test_abandoned_direct_execution_claim_is_durably_expired_and_recovered(
    ...
    def abandoned_execution(**_kwargs):
        solver_entered.set()
    ...
    with TestClient(app) as direct_client, TestClient(app) as recovery_client:
        assert direct_client.post(
            "/planner/workbench/planning-runs",
            json=_phase0_planning_run_payload("RUN-DIRECT-ABANDONED"),
        ).status_code == 200

        with ThreadPoolExecutor(max_workers=1) as executor:
            execution = executor.submit(
                direct_client.post,
                "/planner/workbench/planning-runs/RUN-DIRECT-ABANDONED/execute",
                json={
                    "ExecutedBy": "planner-abandoned",
                    "StartedAt": "2026-06-22T07:55:00+00:00",
                    "TimeLimitSeconds": 1,
                },
            )
            assert solver_entered.wait(timeout=2)
            persisted_claim = store.planning_runs["RUN-DIRECT-ABANDONED"]
            assert "ExecutionClaimExpiresAt" in persisted_claim
            ...
            recovered = recovery_client.post(
                ...
                json={
                    ...
                    "ClaimedAt": claim_expires_at.isoformat(),
                    "LeaseSeconds": 60,
                },
            )
            release_solver.set()
            abandoned_response = execution.result(timeout=5)

    recovered_run = recovered.json()["Data"]["PlanningRun"]
    assert recovered.status_code == 200
    assert recovered_run["RunID"] == "RUN-DIRECT-ABANDONED"
    assert recovered_run["WorkerID"] == "worker-recovery"
    assert recovered_run["RecoveredFromExpiredLease"] is True
    authoritative_run = store.planning_runs["RUN-DIRECT-ABANDONED"]
    assert "ExecutionTokenHash" not in authoritative_run
    assert "ExecutionClaimExpiresAt" not in authoritative_run
    assert authoritative_run["ExecutionClaimInvalidationReason"] == (
        "ExpiredLeaseRecovery"
    )
    assert abandoned_response.status_code == 409
    assert abandoned_response.json()["Data"]["Status"] == (
...
@pytest.mark.parametrize(
    "claimed_at",
    ["not-a-timestamp", "2026-06-22T07:52:00"],
)
def test_worker_claim_rejects_invalid_or_naive_timestamp_before_mutation(
    claimed_at: str,
):
    store = WorkbenchStateStore()
    seed_baseline_test_data(store)
    ...
    assert client.post(
        "/planner/workbench/planning-runs",
        json=_phase0_planning_run_payload("RUN-INVALID-CLAIM-TIME"),
    ).status_code == 200
    assert client.post(
        "/planner/workbench/planning-runs/RUN-INVALID-CLAIM-TIME/enqueue",
        json={
            "EnqueuedBy": "planner-phase0",
            "EnqueuedAt": "2026-06-22T07:51:00+00:00",
        },
...
def test_direct_execution_rejects_naive_started_at_before_claim(monkeypatch):
    ...
    assert client.post(
        "/planner/workbench/planning-runs",
        json=_phase0_planning_run_payload("RUN-NAIVE-DIRECT-CLAIM"),
    ).status_code == 200

    response = client.post(
        "/planner/workbench/planning-runs/RUN-NAIVE-DIRECT-CLAIM/execute",
        json={
            "ExecutedBy": "planner-phase0",
            "StartedAt": "2026-06-22T07:55:00",
            "CompletedAt": "2026-06-22T07:56:00",
        },
    )

    assert response.status_code == 422
    run = store.planning_runs["RUN-NAIVE-DIRECT-CLAIM"]
    assert run["Status"] == "Pending"
    assert "ExecutionTokenHash" not in run
...
def test_get_body_and_revision_header_are_captured_under_one_state_lock():
    body_snapshot_ready = Event()
    allow_reader_return = Event()

    class ReleaseAwareLock:
        def __init__(self, store):
            self.store = store

        def acquire(self, *args, **kwargs):
            return self.store._request_write_lock.acquire(*args, **kwargs)

        def release(self):
            self.store._request_write_lock.release()
            if self.store.arm_reader_release:
                self.store.arm_reader_release = False
                self.store.released_reader_thread = get_ident()

    class HeaderRaceStore(WorkbenchStateStore):
        def __init__(self):
            super().__init__()
            self.arm_reader_release = False
            self.released_reader_thread = None
            self.header_read_after_unlock = Event()
            self.concurrent_save_completed = Event()

        @property
        def state_lock(self):
            return ReleaseAwareLock(self)

        def current_revision(self):
            if self.released_reader_thread == get_ident():
                self.released_reader_thread = None
                self.header_read_after_unlock.set()
                assert self.concurrent_save_completed.wait(timeout=5)
            return super().current_revision()

        def save(self):
            outcome = super().save()
            if self.header_read_after_unlock.is_set():
                self.concurrent_save_completed.set()
            return outcome

    store = HeaderRaceStore()
    reader_app = create_app(state_store=store)
    writer_app = create_app(state_store=store)

    @reader_app.get("/test/revision-pair")
    def read_revision_pair():
        body_revision = store.current_revision()
        body_snapshot_ready.set()
        assert allow_reader_return.wait(timeout=5)
        return {"BodyRevision": body_revision}

    @writer_app.post("/test/revision-pair")
    def advance_revision():
        store.audit_events.append({"Action": "ConcurrentRevisionAdvance"})
        return {"Status": "Advanced"}

    store.arm_reader_release = True
    with TestClient(reader_app) as reader, TestClient(writer_app) as writer:
        with ThreadPoolExecutor(max_workers=2) as executor:
            read = executor.submit(reader.get, "/test/revision-pair")
            assert body_snapshot_ready.wait(timeout=2)
            write = executor.submit(writer.post, "/test/revision-pair")
            allow_reader_return.set()
            read_response = read.result(timeout=5)
            write_response = write.result(timeout=5)

    assert read_response.status_code == 200
    assert write_response.status_code == 200
    assert int(read_response.headers["X-Workbench-Revision"]) == (
...
def test_store_managed_execution_uses_exact_finalization_outcome_revision(
    ...
    class PausingFinalizationStore(WorkbenchStateStore):
        def __init__(self):
            super().__init__()
            self.pause_finalization = True
            self.finalization_committed = Event()
            self.concurrent_save_completed = Event()
            self.finalization_revision = None

        def atomic_update(self, mutation, *, expected_revision=None):
            result, outcome = super().atomic_update(
                mutation,
                expected_revision=expected_revision,
            )
            if (
                self.pause_finalization
                and isinstance(result, dict)
                and set(result) == {"PlanningRun", "ReservationError"}
            ):
                self.pause_finalization = False
                self.finalization_revision = outcome.revision
                self.finalization_committed.set()
                assert self.concurrent_save_completed.wait(timeout=5)
            return result, outcome

        def save(self):
            outcome = super().save()
            if self.finalization_committed.is_set():
                self.concurrent_save_completed.set()
            return outcome

    store = PausingFinalizationStore()
    seed_baseline_test_data(store)
    monkeypatch.setattr("sdbr.api._execute_planning_run", _completed_execution)
    execution_app = create_app(state_store=store)
    writer_app = create_app(state_store=store)

    @writer_app.post("/test/concurrent-finalization-write")
    def concurrent_finalization_write():
        store.audit_events.append({"Action": "AfterFinalization"})
        return {"Status": "Advanced"}
    ...
    with TestClient(execution_app) as execution_client, TestClient(
        ...
        assert execution_client.post(
            "/planner/workbench/planning-runs",
            json=_phase0_planning_run_payload("RUN-EXACT-REVISION"),
        ).status_code == 200

        with ThreadPoolExecutor(max_workers=1) as executor:
            execution = executor.submit(
                execution_client.post,
                "/planner/workbench/planning-runs/RUN-EXACT-REVISION/execute",
                json={
                    "ExecutedBy": "planner-phase0",
                    "StartedAt": "2026-06-22T07:55:00+00:00",
                    "CompletedAt": "2026-06-22T07:56:00+00:00",
            ...
            assert store.finalization_committed.wait(timeout=2)
            advanced = writer_client.post("/test/concurrent-finalization-write")
            executed = execution.result(timeout=5)

    assert advanced.status_code == 200
    assert executed.status_code == 200
    assert int(executed.headers["X-Workbench-Revision"]) == (
        store.finalization_revision
    )
...
def test_concurrent_direct_execution_claims_run_once_and_cannot_overwrite_completed(
    ...
    store = WorkbenchStateStore()
    seed_baseline_test_data(store)
    _seed_active_planning_reservation(store, batch_id="PRB-DIRECT-CAS")
    solver_entered = Event()
    release_solver = Event()
    ...
    with TestClient(app) as first_client, TestClient(app) as second_client:
        assert first_client.post(
            "/planner/workbench/planning-runs",
            json={
                **_phase0_planning_run_payload("RUN-DIRECT-CAS"),
                "PlanningReservationBatchIDs": ["PRB-DIRECT-CAS"],
            },
        ).status_code == 200
        ...
        def second_execute():
            ...
            return second_client.post(
                "/planner/workbench/planning-runs/RUN-DIRECT-CAS/execute",
                json={
                    "ExecutedBy": "planner-phase0-b",
                    "StartedAt": "2026-06-22T07:55:01+00:00",
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            first = executor.submit(
                ...
                "/planner/workbench/planning-runs/RUN-DIRECT-CAS/execute",
                json={
                    "ExecutedBy": "planner-phase0-a",
                    "StartedAt": "2026-06-22T07:55:00+00:00",
                    "CompletedAt": "2026-06-22T07:56:00+00:00",
                },
            )
            assert solver_entered.wait(timeout=2)
            second = executor.submit(second_execute)
    ...
    assert store.planning_runs["RUN-DIRECT-CAS"]["Status"] == "Completed"
    assert store.planning_reservation_batches["PRB-DIRECT-CAS"]["Status"] == (
        "ConvertedToScheduledOccupancy"
    )


def test_concurrent_worker_execution_claims_lease_once_and_keeps_completed_graph(
    monkeypatch,
):
    store = WorkbenchStateStore()
    seed_baseline_test_data(store)
    _seed_active_planning_reservation(store, batch_id="PRB-WORKER-CAS")
    solver_entered = Event()
    release_solver = Event()
    second_request_started = Event()
    calls_guard = Lock()
    ...
    with TestClient(app) as first_client, TestClient(app) as second_client:
        assert first_client.post(
            "/planner/workbench/planning-runs",
            json={
                **_phase0_planning_run_payload("RUN-WORKER-CAS"),
                "PlanningReservationBatchIDs": ["PRB-WORKER-CAS"],
            },
        ).status_code == 200
        assert first_client.post(
            "/planner/workbench/planning-runs/RUN-WORKER-CAS/enqueue",
            json={
                "EnqueuedBy": "planner-phase0",
                "EnqueuedAt": "2026-06-22T07:51:00+00:00",
            },
        ).status_code == 200
        claimed = first_client.post(
            "/planner/workbench/planning-runs/jobs/claim-next",
            json={
                "WorkerID": "worker-phase0",
                "ClaimedAt": "2026-06-22T07:52:00+00:00",
        ...
        def second_execute():
            second_request_started.set()
            return second_client.post(
                "/planner/workbench/planning-runs/RUN-WORKER-CAS/execute",
    ...
    assert sorted(response.status_code for response in responses) == [200, 409]
    assert solver_calls == 1
    assert store.planning_runs["RUN-WORKER-CAS"]["Status"] == "Completed"
...
def test_worker_cannot_finalize_after_authoritative_lease_expiry(monkeypatch):
    ...
    assert client.post(
        "/planner/workbench/planning-runs/RUN-WORKER-EXPIRED-FINALIZE/enqueue",
        json={
            "EnqueuedBy": "planner-phase0",
            "EnqueuedAt": "2026-06-22T07:51:00+00:00",
        },
    ...
    assert response.status_code == 409
    assert response.json()["Data"]["Status"] == "PlanningRunLeaseExpired"
    expired_run = store.planning_runs["RUN-WORKER-EXPIRED-FINALIZE"]
    assert expired_run["Status"] == "Running"
    assert expired_run["ExecutionClaimExpiresAt"] == expired_run["LeaseExpiresAt"]
    assert "ExecutionTokenHash" in expired_run

    recovered = client.post(
        "/planner/workbench/planning-runs/jobs/claim-next",
        json={
            "WorkerID": "worker-after-expiry",
            "ClaimedAt": "2026-06-22T07:52:11+00:00",
            "LeaseSeconds": 60,
        },
    )

    assert recovered.status_code == 200
    assert recovered.json()["Data"]["PlanningRun"]["WorkerID"] == (
        "worker-after-expiry"
    )
    recovered_run = store.planning_runs["RUN-WORKER-EXPIRED-FINALIZE"]
    assert "ExecutionTokenHash" not in recovered_run
    assert "ExecutionClaimExpiresAt" not in recovered_run
    assert recovered_run["ExecutionClaimInvalidationReason"] == (
        "ExpiredLeaseRecovery"
    )
...
def test_planning_reservation_snapshot_and_transition_persist_in_sqlite(
    ...
    database_path = tmp_path / "planning-reservations.db"
    store = SQLiteWorkbenchStateStore(database_path)
    seed_baseline_test_data(store)
    _seed_active_planning_reservation(store, batch_id="PRB-SQLITE")
    store.save()
    monkeypatch.setattr("sdbr.api._execute_planning_run", _completed_execution)
    client = TestClient(create_app(state_store=store))
    assert client.post(
        "/planner/workbench/planning-runs",
        json={
            **_phase0_planning_run_payload("RUN-SQLITE-RES"),
            "PlanningReservationBatchIDs": ["PRB-SQLITE"],
        },
    ).status_code == 200
    assert client.post(
        "/planner/workbench/planning-runs/RUN-SQLITE-RES/execute",
        json={
            "ExecutedBy": "planner-phase0",
            "StartedAt": "2026-06-22T07:55:00+00:00",
            "CompletedAt": "2026-06-22T07:56:00+00:00",
    ...
    assert restored.planning_runs["RUN-SQLITE-RES"]["PlanningReservationBatchIDs"] == [
        "PRB-SQLITE"
    ]
    assert restored.planning_runs["RUN-SQLITE-RES"]["FrozenPlanningReservations"][
        "ReservationBatchIDs"
    ] == ["PRB-SQLITE"]
    assert restored.planning_reservation_batches["PRB-SQLITE"]["Status"] == (
        "ConvertedToScheduledOccupancy"
    )
    ...
    class BarrierFailingSaveStore(WorkbenchStateStore):
        ...
        def save(self) -> None:
            if self.fail_next_save:
                ...
                raise OSError("pre-commit persistence failure")
            super().save()

    store = BarrierFailingSaveStore()
    seed_baseline_test_data(store)
    app = create_app(state_store=store)
    with TestClient(app, raise_server_exceptions=False) as first_client, TestClient(
        app,
        raise_server_exceptions=False,
    ) as second_client:
        store.fail_next_save = True
        with ThreadPoolExecutor(max_workers=2) as executor:
            failed = executor.submit(
                first_client.post,
                "/planner/workbench/planning-runs",
                json=_phase0_planning_run_payload("RUN-ROLLBACK-FAILED"),
            )
            assert store.failure_started.wait(timeout=2)
            succeeded = executor.submit(
                second_client.post,
                "/planner/workbench/planning-runs",
                json=_phase0_planning_run_payload("RUN-ROLLBACK-SUCCEEDED"),
            )
            sleep(0.1)
            store.allow_failure.set()
            failed_response = failed.result(timeout=5)
            succeeded_response = succeeded.result(timeout=5)

    assert failed_response.status_code == 500
    assert succeeded_response.status_code == 200
    assert "RUN-ROLLBACK-FAILED" not in store.planning_runs
    assert store.planning_runs["RUN-ROLLBACK-SUCCEEDED"]["Status"] == "Pending"
...
def test_planning_reservation_workbench_summarizes_copied_business_rows_only():
    ...
    batch = store.planning_reservation_batches["PRB-1"]
